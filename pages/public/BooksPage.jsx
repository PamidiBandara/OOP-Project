...
const BooksPage = () => {
  ...
  useEffect(() => {
    ...
    if (category) {
      setSearchQuery(category) // Pre-fill search if coming from Shop by Category
    }
  }, [location])
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        ...
        // Filter out books: either type is Book, or it has a category (for older data)
        const filteredBooks = allProducts.filter(p => p.type === 'Book' || (!p.type && p.category))
  ...
  return (
    <div className="container-custom py-12 min-h-screen">
      ...
      <p className="text-gray-600 mb-8">Browse our collection of books from various categories.</p>
