...
const StationeryPage = () => {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        ...
        const filtered = allProducts.filter(p => p.type === 'Stationary')
        setItems(filtered)
      } catch (error) {
  ...
  return (
    <div className="container-custom py-12 min-h-screen">
      <h1 className="section-title">Stationery</h1>
      <p className="text-gray-600 mb-8">High-quality stationery for students and professionals.</p>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-500 bg-white rounded-none border border-gray-100 shadow-sm">
          No stationery found matching your criteria. Try adjusting your search!
        </div>
      ) : (
